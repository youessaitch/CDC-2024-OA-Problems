...
// Solution: https://leetcode.com/problems/lexicographically-smallest-beautiful-string/solutions/3488519/c-linear-solution/
...
public:
    void calculate(string &s, int start, int k)
    {
        ...
        { // start from i+1 to n where i is the index jaha mereko palindrome nhi mila to hum ok me ghus gye the
    ...
    string smallestBeautifulString(string s, int k)
    {
        ...
        for (int i = n - 1; i >= 0; i--)
        {
            ...
            for (char ch = s[i] + 1; ch < 'a' + k; ch++)
            {
                bool ok = true; // flag
                if (i - 2 >= 0)
                { // i-1 i-2 dono me se koi palindrome nhi banan chaiye with i
                    if ((ch == s[i - 1]) || (ch == s[i - 2]))
                    {
                        ok = false;
                    }
                ...
                { // i-1 should not form palindrome with i
                    if (ch == s[i - 1])
                    {
                        ok = false;
                    }
